...
static inline bool is_tpu_device(struct gpu_info *device) {
  return device->vendor && device->vendor->name &&
         strcmp(device->vendor->name, "TPU") == 0;
}
...
static void alloc_device_window(unsigned int start_row, unsigned int start_col, unsigned int totalcol,
  ...
  // Line 1 = Name only (TPU version)
  dwin->name_win = newwin(1, totalcol, start_row, start_col);
  if (dwin->name_win == NULL)
    goto alloc_error;

  // These windows are not used for TPU but need to be initialized
  dwin->pcie_info = newwin(1, 1, 0, 0);
  dwin->gpu_clock_info = newwin(1, 1, 0, 0);
  dwin->mem_clock_info = newwin(1, 1, 0, 0);
  dwin->temperature = newwin(1, 1, 0, 0);
  dwin->fan_speed = newwin(1, 1, 0, 0);
  dwin->power_info = newwin(1, 1, 0, 0);

  // Line 2 = TPU used | MEM used (no encoder/decoder for TPU)
  int remaining_cols = totalcol - spacer;
  int size_gpu = remaining_cols / 2;
  int size_mem = remaining_cols - size_gpu;

  // TPU only uses gpu_util_no_enc_and_dec and mem_util_no_enc_and_dec
  dwin->gpu_util_no_enc_and_dec = newwin(1, size_gpu, start_row + 1, start_col);
  if (dwin->gpu_util_no_enc_and_dec == NULL)
    goto alloc_error;
  dwin->mem_util_no_enc_and_dec = newwin(1, size_mem, start_row + 1, start_col + spacer + size_gpu);
  if (dwin->mem_util_no_enc_and_dec == NULL)
    goto alloc_error;

  // Initialize unused windows to dummy 1x1 windows
  dwin->gpu_util_enc_dec = newwin(1, 1, 0, 0);
  dwin->mem_util_enc_dec = newwin(1, 1, 0, 0);
  dwin->encode_util = newwin(1, 1, 0, 0);
  dwin->decode_util = newwin(1, 1, 0, 0);
  dwin->encdec_util = newwin(1, 1, 0, 0);
  dwin->gpu_util_no_enc_or_dec = newwin(1, 1, 0, 0);
  dwin->mem_util_no_enc_or_dec = newwin(1, 1, 0, 0);
  dwin->shader_cores = newwin(1, 1, 0, 0);
  dwin->l2_cache_size = newwin(1, 1, 0, 0);
  dwin->exec_engines = newwin(1, 1, 0, 0);
...
static void initialize_all_windows(struct nvtop_interface *dwin) {
  ...
  // TPU only needs 2 rows: device name + utilization bars
  compute_sizes_from_layout(devices_count, 2, device_length(), rows - 1, cols,
...
static void draw_devices(struct list_head *devices, struct nvtop_interface *interface) {
  ...
  list_for_each_entry(device, devices, list) {
    ...
    // Line 2: TPU and MEM utilization bars
    WINDOW *gpu_util_win = dev->gpu_util_no_enc_and_dec;
    WINDOW *mem_util_win = dev->mem_util_no_enc_and_dec;
    char buff[1024];

    // TPU utilization
    if (GPUINFO_DYNAMIC_FIELD_VALID(&device->dynamic_info, gpu_util_rate)) {
      snprintf(buff, 1024, "%u%%", device->dynamic_info.gpu_util_rate);
      draw_percentage_meter(gpu_util_win, "TPU", device->dynamic_info.gpu_util_rate, buff);
    } else {
      snprintf(buff, 1024, "N/A");
      draw_percentage_meter(gpu_util_win, "TPU", 0, buff);
    }
    ...
    if (GPUINFO_DYNAMIC_FIELD_VALID(&device->dynamic_info, total_memory) &&
      ...
      snprintf(buff, 1024, "%.3f%s/%.3f%s", used_prefixed, memory_prefix[prefix_off],
               total_prefixed, memory_prefix[prefix_off]);
...
static const char *columnName[process_field_count] = {
    "PID", "USER", "DEV", "TYPE", "TPU", "ENC", "DEC", "TPU MEM", "CPU", "HOST MEM", "Command",
};
